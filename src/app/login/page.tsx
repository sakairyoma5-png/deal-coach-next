...
export default function LoginPage() {
  ...
  // Build Supabase OAuth URL directly — no JS SDK call needed
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
  const redirectTo = typeof window !== "undefined"
    ? `${window.location.origin}/auth/callback`
    : "";
  const googleLoginUrl = `${supabaseUrl}/auth/v1/authorize?provider=google&redirect_to=${encodeURIComponent(redirectTo)}`;
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="w-full max-w-sm p-8">
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h1 className="text-2xl font-bold tracking-tight">DealCoach</h1>
